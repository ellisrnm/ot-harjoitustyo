...
class BugView:
    ...
    def _initialize(self):
        ...
        text=f"Tarkastelet projektin {self._project} aliprojektia {self._subproject}",
        font='Arial 20 bold')

        (total, new, under_development, on_hold, fixed,
        current) = bugtracker_service.get_bug_statistics(self._project, self._subproject)
        ...
        title_unfixed = ttk.Label(master=self._frame,
        text="Korjaamatta olevat bugit",
        font='Arial 20 bold')

        label_totals_new = ttk.Label(master=self._frame, text=f"Uusia: {new}")
        label_totals_develop = ttk.Label(master=self._frame, text=f"Kesken: {under_development}")
        ...
        if bugs:
            subtitle_bugs = ttk.Label(master=self._frame,
            text="Bugien tiedot",
            font='Arial 14 bold')
            subtitle_bugs.pack(pady=(10, 10))
            for bug in bugs:
                prio = bugtracker_service.convert_priority(bug.priority)
                stat = bugtracker_service.convert_status(bug.status)
                label_bug_info = ttk.Label(master=self._frame,
                text=f"{bug.name}, Prioriteetti: {prio}, Tila: {stat}")
                label_bug_info.pack(pady=(0, 0))

        title_update = ttk.Label(master=self._frame,
        text="Muokkaa tietoja",
        font='Arial 20 bold')

        desc_update_1 = ttk.Label(master=self._frame,
        text="Voit muokata korjaamattamon bugin tilaa ja prioriteettia",
        font='Arial 14')

        desc_update_2 = ttk.Label(master=self._frame,
        text="Valitse alta bugi, jonka tietoja haluat muuttaa.",
        font='Arial 14')
        ...
        if not bug_options:
            label_nobugs = ttk.Label(master=self._frame,
            text="Huom! Tällä aliprojektilla ei ole avoimia bugiraportteja")
            ...
            subtitle_update_status = ttk.Label(master=self._frame,
            text="Muuta tilastatus",
            font='Arial 14 bold')
            ...
            statusbutton = ttk.Button(master=self._frame,
            text="Päivitä status",
            command=self._update_status_handler)

            subtitle_update_status.pack(pady=(10, 5))
            statuschoose.pack(pady=(5, 0))
            statusbutton.pack(pady=(0, 10))

            subtitle_update_priority = ttk.Label(master=self._frame,
            text="Muuta prioriteettia",
            font='Arial 14 bold')

            priority_options = ["Vähäinen", "Normaali", "Korkea"]
            self._priority_chosen.set(priority_options[0])

            prioritychoose = OptionMenu(self._frame,
            self._priority_chosen,
            *priority_options)
            prioritybutton = ttk.Button(master=self._frame,
            text="Päivitä prioriteetti",
            command=self._update_priority_handler)

            subtitle_update_priority.pack(pady=(10, 5))
            prioritychoose.pack(pady=(5, 0))
            prioritybutton.pack(pady=(0, 10))

        title_create_new = ttk.Label(master=self._frame,
        text="Luo uusi",
        font='Arial 20 bold')

        desc_create_new_1 = ttk.Label(master=self._frame,
        text="Voit lisätä uuden bugin kirjoittamalla sille nimen.",
        font='Arial 14')

        desc_create_new_2 = ttk.Label(master=self._frame,
        text="Nimi ei voi olla sama kuin jollain toisella raportoidulla bugilla.",
        font='Arial 14')

        self._entry_bug = ttk.Entry(master=self._frame)

        button_create_new = ttk.Button(master=self._frame,
        text="Raportoi",
        command=self._create_bug_handler)
